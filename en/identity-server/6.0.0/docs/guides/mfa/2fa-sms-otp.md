...
Optionally, you can configure backup codes as a fallback authentication method when SMS OTP is temporarily inaccessible (for example, when a user cannot receive the SMS). Note that backup codes work only when SMS OTP is enabled, not as a replacement for it.
